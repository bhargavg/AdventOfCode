...
func part1(input: [String]) -> Int {
    return input
        .lazy
        .map(evaluateWithSamePrecedence(expression:))
        .reduce(0, +)
}
...
func part2(input: [String]) -> Int {
    return input
        .lazy
        .map(evaluateWithDifferentPrecendence(expression:))
        .reduce(0, +)
}


func evaluateWithSamePrecedence(expression: String) -> Int {
    var startIndex = expression.startIndex
    return evaluateWithSamePrecendence(expression: expression, index: &startIndex)
}

func evaluateWithSamePrecendence(expression: String, index: inout String.Index) -> Int {
    var res = 0
    var lastOp = "+"
    ...
    while index < expression.endIndex {
        ...
        case "*":
            lastOp = "*"
            _ = expression.formIndex(&index, offsetBy: 1, limitedBy: expression.endIndex)
        ...
        default:
            ...
            if expression[index].isNumber {
                while index < expression.endIndex && expression[index].isNumber {
                    num = (num * 10) + Int(expression[index].asciiValue! - 48)
                    expression.formIndex(after: &index)
                }

            } else if expression[index] == "(" {
                _ = expression.formIndex(&index, offsetBy: 1, limitedBy: expression.endIndex)
                num = evaluateWithSamePrecendence(expression: expression, index: &index)
            } else {
                fatalError()
            }
...
func evaluateWithDifferentPrecendence(expression: String) -> Int {
    var startIndex = expression.startIndex
    return evaluateWithDifferentPrecendence(expression: expression, index: &startIndex)
}

func evaluateWithDifferentPrecendence(expression: String, index: inout String.Index) -> Int {
    var stack = [Int]()
    var lastOp = "+"
    ...
    while index < expression.endIndex {
        ...
        if expression[index] == "*" {
            ...
            if expression[index].isNumber {
                ...
                _ = expression.formIndex(&index, offsetBy: 1, limitedBy: expression.endIndex)
                num = evaluateWithDifferentPrecendence(expression: expression, index: &index)
            } else {
                fatalError()
            }
            ...
            switch lastOp {
            case "+":
                stack.append((stack.popLast() ?? 0) + num)
            case "*":
                stack.append(num)
            default:
                fatalError()
...
assert(part1(input: input) == 7147789965219)
assert(part2(input: input) == 136824720421264)
